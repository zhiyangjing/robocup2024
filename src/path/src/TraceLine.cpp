...
TraceLine::TraceLine(int remain_time, ros::NodeHandle &nh) : Ability(remain_time, nh) {
    ...
    blue_line_slopes = Buffer<float>(1);
    ...
    x << -0.5, -0.776, -0.4, -0.46, -0.584, -0.70, -0.51, -0.48, -1.397, 0, 0, -1.25, 0, -0.6, 0, 0, -0.6, -0.41, -0.58,
        -0.7, -0.4, -46;
    y << 0.620, 0.43, 0.84, 0.753, 0.575, 0.496, 0.51, 0.578, 0, 1.4, 1.2, 0, 0.67, 0.5, 0.52, 0.63, 0, 0, 0, 0, 0, 0;
    c << 300, 460, 206, 247, 350, 400, 471, 217, 450, 246, 184, 505, 215, 310, 310, 240, 497, 343, 310, 339, 405, 440;
    z << 0, 0, 0, 0, 0, 0, 150, -170, 120, -120, -230, 200, -230, 0, 0, -170, 200, 150, 0, 0, 230, 230;
...
void TraceLine::linePreprocess() {
    if (lines_raw.empty()) {
        ROS_WARN(TAG "No lane line to preprocess");
    } else {
        posLines.clear();
        negLines.clear();
...
void TraceLine::lineSlopeStrategy(float left_slope, float right_slope, int center) {
    if (blue_line_found and blue_line_visible and not dir_adjust_finish) {
        ...
        if (fabs(slope) < 0.01) {
            ...
            ROS_INFO(TAG BCOLOR_YELLOW "b and not dir_adjust_finishlue slope: %f angle: %d" COLOR_RESET, slope, -170);
